...
const Chart = ({ chart }) => {
  ...
  const likedIcon = isLiked
    ? `fa-solid fa-heart text-[#FACD66]`
    : `fa-regular fa-heart text-[#FACD66]`;
  ...
  return (
    <div className="bg-[#0505052c] flex text-white p-3 lg:items-center shrink-0 rounded-3xl mb-5">
      <Link to="/Playlists">
        <div className="lg:flex" onClick={() => saveDivId(chart.id)}>
          <img
            ...
            alt="image"
            className="mr-5 mb-3 lg:mb-0 w-[108px] h-[99px] lg:w-16 lg:h-16"
          />
          <div className="mr-24 ">
            <p className="text-lg w-36 md:w-44 lg:w-52 text-gray-200 ">
              {chart.name}
            </p>
            <p className="text-xs text-gray-400 mb-4 lg:mb-2">
      ...
      <div className="relative ">
        ...
        <i
          className={`${likedIcon} absolute top-[30%] right-[30%] cursor-pointer hover:scale-110`}
          onClick={() => handleLiked(chart.id)}
        />
      </div>
    </div>
  );
