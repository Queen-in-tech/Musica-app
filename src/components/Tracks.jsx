const Tracks = ({tracks, album}) => {
  return (
    ...
    {tracks.map((track) => {
        ...
        return <div className="w-full sm:w-[85vw]">
        <div className="flex text-gray-300 bg-[#33373e42] px-3 py-2 items-center rounded-xl mb-3 sm:grid sm:grid-flow-col grid-cols-6">
            ...
            <p className="hidden sm:block font-light text-sm col-span-1">4:35</p>
            <i class="fa-solid fa-ellipsis-vertical text-[#FACD66] hidden sm:block text-sm col-span-1 text-right"></i>
            <div className="w-full sm:hidden">
                <p className="font-light text-sm mb-1">{`${track.name} - ${track.artists[0].name}`}</p>
