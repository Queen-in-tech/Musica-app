...
const AlbumsDetails = ({ track, album }) => {
  ...
  const isAlreadyLiked = isInFavourite
    ? "fa-solid fa-heart"
    : "fa-regular fa-heart";
    ...
    <div className="min-h-screen mx-auto lg:mx-0 shrink md:shrink-0">
      <div className="flex flex-col sm:flex-row sm:gap-8 font-body md:mx-0">
        <img
          ...
          alt="album-img"
          className="rounded-lg w-[350px] mx-auto sm:mx-0 sm:w-80 sm:mb-8"
        />
        <div className="text-white mt-3 sm:mt-32 pl-2">
          <p className="text-4xl text-[#A4C7C6] font-bold capitalize mb-3 max-w-sm md:max-w-md">
            {album.name}
          </p>
          <p className="text-xs mb-1 text-gray-300 w-[340px] sm:w-80 md:w-96 lg:w-[564px]">
          ...
          <div className="flex gap-6 mb-5">
            <div className="bg-[#33373B] py-2 px-3 flex rounded-xl items-center cursor-pointer">
              <i
                class={`${playPauseIcon} mr-3 text-[#FACD66] cursor-pointer`}
              ></i>
              <span
                className="text-xs font-light"
                onClick={() => handleCurrentTrack(track.items[0])}
              >{`${playPauseText}`}</span>
            </div>
            <div className="bg-[#33373B] py-2 px-3 rounded-xl flex items-center cursor-pointer">
              <i
                class={`${isAlreadyLiked} mr-3 text-[#FACD66] text-xs cursor-pointer`}
                onClick={() => favourite(album.id)}
              ></i>
